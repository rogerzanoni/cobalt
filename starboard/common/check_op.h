...
class CheckOpResult {
 public:
  ...
  constexpr explicit CheckOpResult(logging::LogMessage* log_message)
      : log_message_(log_message) {}

  // Returns true if the check succeeded.
  constexpr explicit operator bool() const { return !log_message_; }

  logging::LogMessage* log_message() { return log_message_; }
  ...
  static logging::LogMessage* CreateLogMessage(const char* file,
                                               int line,
                                               const char* expr_str,
                                               char* v1_str,
                                               char* v2_str);
 ...
 private:
  ...
  logging::LogMessage* const log_message_ = nullptr;
};
