...
// bionic's mkostemp rejects a flags argument that includes the access mode or
// O_CREAT/O_EXCL with EINVAL (only O_APPEND, O_CLOEXEC and O_SYNC are
// permitted), which musl callers pass. Reimplement musl's mkostemp via openat()
// so those flags are accepted.
